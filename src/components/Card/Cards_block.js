  ...
  const pushFilters = () => {
    statsFilter.forEach((el) => {
      ...
      if (el.attackFrom || el.attackTo)
        statsFilterObj["attack"] = {
          from: el.attackFrom || 0,
          to: el.attackTo || 99999,
        };
      if (el.defenseFrom || el.defenseTo)
        statsFilterObj["defense"] = {
          from: el.defenseFrom || 0,
          to: el.defenseTo || 99999,
        };
      if (el.specialattackFrom || el.specialattackTo)
        statsFilterObj["specialAttack"] = {
          from: el.specialattackFrom || 0,
          to: el.specialattackTo || 99999,
        };
      if (el.specialdefenseFrom || el.specialdefenseTo)
        statsFilterObj["specialDefense"] = {
          from: el.specialdefenseFrom || 0,
          to: el.specialdefenseTo || 99999,
        };
      if (el.speedFrom || el.speedTo)
        statsFilterObj["speed"] = {
          from: el.speedFrom || 0,
          to: el.speedTo || 99999,
        };
    });
